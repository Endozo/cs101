...
int main() {
    ...
    emp_record_t employee;

    printf("Please enter operators number(0-5): ");
    scanf("%d", &sid);
    if (sid == 0) {
        printf("Please enter which new ID do you want to add (1~99) : ");
        scanf("%d", &employee.emp_id);
        printf("Please enter the name of the new employee: ");
        scanf("%s", &employee.emp_name);
        printf("Please enter the employee's salary: ");
        scanf("%d", &employee.emp_salary);
        FILE* fp;

        fp = fopen("operator_id.bin", "append(ab)");

        fprintf(fp, "================\n");
        fprintf(fp, "Operator's ID: %d\n", employee.emp_id);
        fprintf(fp, "Operator's name: %s\n", employee.emp_name);
        fprintf(fp, "Operator's salary: %d\n", employee.emp_salary);

        fclose(fp);
    } else {
        printf("How many set do you want to buy: ");
        scanf("%d", &set);
        yourlotto.lotto_receipt = set * 50 * 1.1;
        ...
        fwrite(&yourlotto, sizeof(yourlotto), 1, file);
